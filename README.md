...
“重连窗口”是 OpenConnect 自身处理 SSL/DTLS 短暂掉线的最长时间。若服务端以 `Idle Timeout` 结束整个会话，应用层的“会话退出后自动重连”会在隧道曾成功建立的前提下，按“自动重连间隔”重新启动 OpenConnect。连续失败 10 次后停止，避免无效凭据持续触发认证；点击“断开连接”也会立即取消待执行的重连。
