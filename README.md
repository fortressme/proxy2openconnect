...
“重连窗口”是 OpenConnect 自身处理 SSL/DTLS 短暂掉线的最长时间。若服务端以 `Idle Timeout` 结束整个会话，应用层的“会话退出后自动重连”会在隧道曾成功建立的前提下，按 Web 控制台设置的“自动重连间隔”和“自动重连尝试次数”重新启动 OpenConnect。尝试次数可设置为 1–100，默认为 5；达到上限后停止，避免无效凭据持续触发认证。点击“断开连接”也会立即取消待执行的重连。
...
“运行总览”展示 `tun0` 的累计字节数、实时收发速率、数据包与错误数，以及 VPN/Xray 运行时间、路由、DNS、保活和自动重试状态。当前连接区域展示 Xray 实际连接的远端目标地址，并依据连接使用的本地源地址区分 VPN、直连和未识别流量；Xray 日志中的 outbound 标签仅作为辅助信息。

目标历史只在 VPN 隧道在线时统计 Xray 新建的目标 TCP 连接。每条新建连接计为一次访问；它不是 HTTP 请求数，也不包含 UDP。历史按 VPN 服务器、用户名和认证组生成的不可逆标识独立归档，因此切换 VPN 配置后只会看到对应配置的记录。控制台支持按域名、IP、端口或 outbound 搜索，按 VPN、直连和未识别路径筛选，并按访问频次、最近访问、活跃天数或目标名称排序。
